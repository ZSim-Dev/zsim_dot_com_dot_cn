<script setup lang="ts">
  ...
  async function handleVote(character: Character) {
    ...
    try {
      await axios.post(
        `/api/vote/character/${character.id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token.value}` },
        }
  ...
  <div class="vote-page">
    ...
    <div class="category-switcher">
      <button
        ...
        :key="category.key"
        :class="['category-btn', { active: selectedCategoryKey === category.key }]"
      ...
      <transition name="fade-view" mode="out-in">
        <div v-if="selectedCategoryKey === 'character'" class="character-grid">
          <div v-for="char in characters" :key="char.id" class="character-card">
            ...
            <button
              class="vote-btn"
              @click="handleVote(char)"
              :disabled="votedCharacterIds.includes(char.id)"
            >
